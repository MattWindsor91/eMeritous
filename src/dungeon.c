...
int play_dungeon(char *fname)
{
  ...
  game_running = 1;
  while (game_running) {
    /*sprintf(buf, "X: %d  Y: %d", (player.x + PLAYER_W/2)/32*32 + PLAYER_W/2, (player.y + PLAYER_H/2)/32*32 + PLAYER_H/2);
    ...
    if (!game_paused) {
      if (player_dying > 30) {
        ...
        if (player.hp <= 0) {
          ...
          if ( (current_boss == BOSS_FINAL) && (boss_fight_mode != 0) ) {
    ...
    if (player_room != prv_player_room) {
      ...
      if (((rooms[player_room].checkpoint) || (player_room == 0)) && 
    ...
    if (last_killed != killed_enemies) {
      ...
      if ((player_room == 0) && (artifacts[ART_CURSED_SEAL] == 1)) {
        SetTonedPalette(0);
      }
    }
    ...
    if (!map_enabled) {
      ...
      if (player_dying == 0) {
        ...
        if (magic_circuit > 0) {
          if (player_dying == 0) {
            if (circuit_release == 0) {
              ...
              for (i = 0; i < n_arcs; i++) {
                Arc(screen, player.x - scroll_x + PLAYER_W/2, player.y - scroll_y + PLAYER_H/2, circuit_range, arcdir);
        ...
        if (t % 2 == 0) DrawPlayer((SCREEN_W / 2) - (PLAYER_W / 2), 
                                   (SCREEN_H / 2) - (PLAYER_H / 2),
                                   player_dir, player_wlk / wlk_wait);
      ...
      if ((rooms[player_room].room_type == 4) && 
      ...
      if (circuit_release > 0) {
        DrawCircle(release_x - player.x + 320, release_y - player.y + 240, circuit_release * release_range / 20, sin((float)circuit_release / 20.0)*127+127);
        if (!game_paused) {
      ...
      if (!game_paused) {
        if (shield_hp < player.reflect_shield) {
          shield_recover += player.reflect_shield * 3 / (3 - training - player.has_agate_knife);
          if (artifacts[ART_SHIELD_BOOSTER]) {
            shield_recover += (player.reflect_shield * 3 / 
                               (3 - training - (player.has_agate_knife)));
          }
    ...
    SpecialTile((player.x+PLAYER_W/2)/32, (player.y+PLAYER_H/2)/32);
    ...
    if ((player_dying != 0) && (player.hp <= 1)) can_move = 0;
    if (rooms[player_room].room_type == 5)
      if (CanGetArtifact())
    ...
    if (can_move) {
      ...
      if ((key_held[K_SP])&&(magic_circuit >= 0)) {
        magic_circuit += (player.circuit_charge * (3+training+(player.circuit_charge==30))/3);
      } else {
        if (magic_circuit < 0) {
          magic_circuit += (player.circuit_refill * (3+training+(player.circuit_refill==30))/3);
      ...
      if ((ix!=player.x)||(iy!=player.y)) {
        ...
        if (((player.x / 32)!=((ix+off_x) / 32)) || ((player.y / 32)!=((iy+off_y) / 32))) {
          /* printf("%d\n", tile) */;
          if (TouchTile(ix, iy)) {
