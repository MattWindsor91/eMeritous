...
int searched[NUM_ROOMS];
int searchdist[NUM_ROOMS];
int csearch;
int room_active[NUM_ROOMS];
...
{
  if (v == 0) return;
  if ( (rand()%1000) < ((int)log(v)/4 + (player.hp == 1)*5 + 2) ) {
    SCreateGem(x, y, r, GEM_HEART);
  } else {
    SCreateGem(x, y, r, v);
  }
}
...
{
  float dy = player.y+12 - y;
  float dx = player.x+8 - x;
  return atan2(dy, dx);
}

int PlayerDist(int x, int y)
{
  int d = sqrt(sqr(x-(player.x+8))+sqr(y-(player.y+12)));
  return d;
}
...
void InitEnemySprites()
{
  ...
  for (i = 0; i < 10; i++) {
    sprintf(buf, "dat/i/mons%u.png", i + 1);
    enemy_sprites[i] = IMG_Load(buf);
    SDL_SetColorKey(enemy_sprites[i], SDL_SRCCOLORKEY | SDL_RLEACCEL, 255);
  }
...
void MoveBullet(struct bullet *e)
{
  ...
  if ((e->dying == 1) && (e->img == 8)) {
    SpawnLaser(e->x - cos(e->dir) * e->speed, e->y - sin(e->dir) * e->speed, PlayerDir(e->x, e->y), 10, 10, 0.0, player.reflect_shield / 6);
  }
  ...
  if (b->img == 2) {
    ...
    if (b->t <= b->fire_time) {
      ...
      while (!IsSolid(Get((fx)/32, (fy)/32))) {
        ...
        if ((player.reflect_shield > 0)&&(shield_hp > 0)&&(PlayerDist(fx, fy) < 30)) break;
          ...
          if (g->value == GEM_HEART) {
            ...
            if (player.hp < (3 + (player.has_agate_knife) * 3)) {
              ...
              if (!training) {
                ...
                /* Add a life if enough life parts are collected. */
...
void SpawnBullet(int x, int y, int bullet_type, float dir, float spd, int invuln)
{
  ...
  if ( (current_boss == 3) && (player.reflect_shield == 30) && (boss_fight_mode == 2) ) {
    spd *= 1.2;
  }
