...
int current_boss = BOSS_MERIDIAN;
int boss_x, boss_y;
int boss_hp;
int boss_lives;
int boss_tail_len = 0;
  ...
  /*   "This string of text is precisely seventy eight characters in length, usefully." */
  {
    { /* Meridian */
      ...
      {0, 6, 1,
       ...
       "  the surface, MERIDIAN shall return you to the dust whence you came!"
      }
    },
...
void BossDialog()
{
  ...
  if (current_boss == BOSS_FINAL) {
    if (player.has_agate_knife) {
      dialog_set = BOSS_FINAL + 1;
    }
  }
...
void DrawPowerObject()
{
  ...
  n_artifacts = (current_boss + 
                 artifacts[ART_HOLY_SWORD] + 
                 artifacts[ART_MYSTIC_HALBERD] + 
                 artifacts[ART_DIVINE_BOW]);
      ...
      if (CanGetArtifact()) {
        if ((Get((player.x+PLAYER_W/2)/32, (player.y+PLAYER_H/2)/32)==42) ||
          ...
          if (rooms[player_room].enemies == 0) {
            ...
            /*DrawCircle(p_x + 16 - scroll_x, player.y - 48 - scroll_y, collect * 4 + 1, rand()%192+64); */
            DrawCircle(player.x + PLAYER_W/2 - scroll_x, player.y + PLAYER_H/2- scroll_y, (100-collect) * 4 + 1, rand()%192+64);
...
{
  if (!IsSolid(Get((move_x - 12 + (current_boss == BOSS_FINAL) * 6) / 32,
                   (move_y - 12 + (current_boss == BOSS_FINAL) * 4) / 32))) {
    if (!IsSolid(Get((move_x + 12 - (current_boss == BOSS_FINAL) * 6) / 32,
                     (move_y - 12 + (current_boss == BOSS_FINAL) * 4) / 32))) {
      if (!IsSolid(Get((move_x - 12 + (current_boss == BOSS_FINAL) * 6) / 32,
                       (move_y + 12 - (current_boss == BOSS_FINAL) * 4) / 32))) {
...
void BC_BossCombat()
{
  ...
  case BOSS_MERIDIAN: {
    ...
    case 3: {
      ...
      laser_dmg = (rand()%(player.reflect_shield/2+1))/2;
					
      for (i = 0; i < player.reflect_shield / 5 + 1; i++) {
    ...
    case 2: {
      ...
      laser_dmg = (rand()%(player.reflect_shield/2+1))/2;
      for (i = 0; i < player.reflect_shield / 3 + 1; i++) {
    ...
    case 1: {
      ...
      laser_dmg = (rand()%(player.reflect_shield/2+1))/2;
      for (i = 0; i < player.reflect_shield / 4 + 1; i++) {
        SpawnBullet(boss_x + (rand() % 48)*pdir_x, boss_y + (rand() % 48)*pdir_y, 3, RandomDir(), 3.5, 0);
  ...
  case BOSS_ATARAXIA: {
    ...
    if ((t % (100 + boss_lives * 50)) == ((100 + boss_lives * 50)-1)) {
      ...
      for (j = 0; j < i; j++) {
        SpawnLaser(boss_x + cos(hd2*j) * 20, boss_y + sin(hd2*j) * 20, pdir, 15 + rand()%16, 5, 0, (player.reflect_shield / 3 + 1));
      }
    }
    ...
    if (boss_lives == 2) {
      for (i = 0; i < heads; i++) {
        for (j = 0; j < player.reflect_shield / 6 + 1; j++) {
    ...
    if (boss_lives <= 2) {
      if ((t % (12 + (boss_lives - 1) * 30)) == 9) {
        for (i = 0; i < heads; i++) {
          SpawnLaser(hx[i], hy[i], pdir, 6, 6, 0, (player.reflect_shield / 6 + 1));
        }
      }
    }
    break;
  ...
  case BOSS_MERODACH: {
    ...
    if (boss_m_heads < 4) {
      if ((t % 5) == 0) {
        i = (t / 5)%boss_m_heads;
        md = boss_m_hd[i];
        SpawnLaser(boss_m_hx[i], boss_m_hy[i], md, 1, 10, 0.05, (player.reflect_shield / 7) + 3 - boss_m_heads);
        md += M_PI / 2;
        SpawnLaser(boss_m_hx[i], boss_m_hy[i], md, 1, 10, 0.05, (player.reflect_shield / 7) + 3 - boss_m_heads);
        md += M_PI / 2;
        SpawnLaser(boss_m_hx[i], boss_m_hy[i], md, 1, 10, 0.05, (player.reflect_shield / 7) + 3 - boss_m_heads);
  ...
  case BOSS_FINAL:
    {
      ...
      mx = boss_x + cos(boss_dir)*(4 + (player.reflect_shield==30));
      my = boss_y + sin(boss_dir)*(4 + (player.reflect_shield==30));
      ...
      if ((boss_lives == 1) && (player.has_agate_knife)) {
        {
          int room_w, room_h;
          int room_x, room_y;
          float tmr_t;
      ...
      boss_loop_total = 1 + (player.has_agate_knife)*2;
      ...
      for (boss_loop = 0; boss_loop < boss_loop_total; boss_loop++) {
        ...
        case 4:
          ...
          if ((t % 10) == 3) {
            ...
            SpawnLaser(ls_x, ls_y, firedir, 8, 2, 0, player.reflect_shield/2+1);
          }
        ...
        case 5:
          ...
          if ((t % 8) == 7) {
            ...
            firedir = PlayerDir(ls_x, ls_y) - 0.1 + ((float)(rand()%16))/8.0;
            SpawnLaser(ls_x, ls_y, firedir - 0.03*6, 5, 5, 0.03, player.reflect_shield/3+1);
          }
    ...
    if (boss_fight_mode == 3) {
      ...
      if (bdef > 0) {
        ...
        if (t_timer < 220) {
          ...
          for (i = 0; i < 10; i++) {
            rt = (rand() % 350) + 50;
            DrawCircle(player.x + PLAYER_W/2 - scroll_x + cos(dr)*rt, player.y - scroll_y + PLAYER_H/2
                       + sin(dr)*rt, rand()%30+5, rand()%128+128);
          }
					
          dr -= 0.025;
          for (i = 0; i < 5; i++) {
            Arc(screen, player.x + PLAYER_W/2 - scroll_x, player.y + PLAYER_H/2 - scroll_y, 450, dr);
            dr += 0.01;
          }
          dr += 0.03;
          ...
          DrawCircle(player.x - scroll_x + PLAYER_W/2, player.y - scroll_y + PLAYER_H/2, (t_timer % 30) * 15, 255);
          DrawCircle(player.x - scroll_x + PLAYER_W/2, player.y - scroll_y + PLAYER_H/2, ( (t_timer + 15) % 30) * 15, 255);
        } else {
          magic_circuit = 0;
    ...
    if (circle_size < 128) {
      ...
      if (circle_size2 > 4) {
        ...
        if (boss_lives > 0) {
          boss_new_life = 0;
          boss_hp = 1000;
          if ( (boss_lives == 1) && (current_boss == 3) && (player.has_agate_knife) ) {
            player.hp = 6;
            boss_dmgrate = 0.2;
          }
        } else {
...
void DrawArtifactOverhead(int p_obj)
{
  ...
  p_x = player.x - 8;
  p_y = player.y - 36 + sin((float)tick / 20.0)*4;
	
  from.x = (ART_HOLY_SWORD + p_obj) * 32;
  from.y = 0;
  from.w = 32;
  from.h = 32;
...
void BossRoom(int room)
{
  ...
  case BOSS_MERODACH:
    ...
    break;
  case BOSS_FINAL:
    boss_lives = 2 + (player.has_agate_knife);
    boss_breakpoint = 120;
    boss_dmgrate = 0.40 + 0.10*(player.has_agate_knife);
